...
impl GcArgs {
    pub async fn run(&self, _scope: Option<String>) -> Result<()> {
        let registry = rms_memory_core::config_manager::load_registry()?;
        let dbs_dir = rms_memory_core::workspace::base_dir().join("dbs");
        if !dbs_dir.exists() {
        ...
        for project in registry.projects.values() {
            let canonical = std::fs::canonicalize(&project.vault_path)
                .unwrap_or_else(|_| std::path::PathBuf::from(&project.vault_path));
        ...
        if confirm {
            ...
            println!("GC complete. Deleted {deleted} orphaned databases.");
        } else {
