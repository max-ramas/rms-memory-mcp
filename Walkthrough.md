...
- **Editor-agnostic hooks (L3):** `rms-memory hook --event session_start|pre_compact|session_stop` prints machine-readable JSON; `--apply` creates/updates or closes a checkpoint. `rms-memory install` writes thin adapters: managed Cursor `~/.cursor/hooks.json` entries, plus shared/Claude/Neovim scripts that only invoke the CLI. Executable paths are embedded as **single-quoted** bash literals (`shell_single_quote`) so `$`, backticks, spaces, and quotes from `current_exe`/CLI install paths cannot expand. Invalid existing `hooks.json` triggers `tracing::warn!` + console warning before `.bak` rewrite. Uninstall strips managed hooks without touching foreign entries.
