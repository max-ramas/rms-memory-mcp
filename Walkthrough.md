...
- **Separate corpora:** Markdown and derived Rust source live in independent LanceDB tables. `reindex --vault` (the default), `reindex --code`, and `reindex --all` make the refresh target explicit; code indexing never modifies repository sources.
- **Semantic Rust chunks:** A tree-sitter parser emits module, type, trait, implementation, function, and related items. Large items are split with their enclosing module/impl preamble and declaration signature repeated in each segment, so a retrieved method remains interpretable outside its original file.
